...
$("#matchings-wrapper").on("click", ".pay-matching-btn ", function () {
    ...
    alert("뭔데")
    // 모달에 정보 옮겨놓기
    const matchingDate = $(this).closest('.matching-wrapper').find('.matching-date-info').text();
    const matchingTime = $(this).closest('.matching-wrapper').find('.matching-time-info').text();
    const matchingField = $(this).closest('.matching-wrapper').find('.matching-field-info').text();
    ...
    const matchingSeq = $(this).closest('.matching-wrapper').attr('data-matchingSeq');
